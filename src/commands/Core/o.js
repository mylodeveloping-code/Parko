        ...
        try {
            ...
            if (hasRole) {
                await member.roles.remove(
                    ROLE_ID,
                    `Owner role toggled off by ${interaction.user.tag}.`
                );
                ...
                return interaction.reply({
                    embeds: [
                        createEmbed({
                            title: '👑 Owner',
                            description:
                                `The **Owner** role has been removed from ${member}.`,
                            color: 'warning',
                        }),
                    ],
                    ephemeral: true,
            ...
            await member.roles.add(
                ROLE_ID,
                `Owner role toggled on by ${interaction.user.tag}.`
            );
            ...
            return interaction.reply({
                embeds: [
                    createEmbed({
                        title: '👑 Owner',
                        description:
                            `The **Owner** role has been added to ${member}.`,
                        color: 'success',
                    }),
                ],
                ephemeral: true,
    ...
    async messageExecute(message, args) {
        ...
        try {
            ...
            if (hasRole) {
                await member.roles.remove(
                    ROLE_ID,
                    `Owner role toggled off by ${message.author.tag}.`
                );
            } else {
                await member.roles.add(
                    ROLE_ID,
                    `Owner role toggled on by ${message.author.tag}.`
                );
            }
        } catch (error) {
            console.error(
